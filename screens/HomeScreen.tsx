...
interface SuggestedPlaylist {
  ...
  icon: keyof typeof Ionicons.glyphMap; query: string; gradient: [string, string];
}
...
export default function HomeScreen() {
  ...
  const loadTrending = async () => {
    try { const songs = await getChart(); setTrending(songs); } catch (e) { console.error(e); }
    setLoading(false);
  };

  const handlePlaylistPress = async (playlist: SuggestedPlaylist) => {
    if (playlistSongs[playlist.id]?.length) { const songs = playlistSongs[playlist.id]; playSong(songs[0], songs); return; }
    setLoadingPlaylist(playlist.id);
    try { const songs = await searchSongs(playlist.query); if (songs.length > 0) { setPlaylistSongs((p) => ({ ...p, [playlist.id]: songs })); playSong(songs[0], songs); } } catch (e) { console.error(e); }
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={[s.contentContainer, { paddingTop: insets.top + Spacing.sm }]}>
      <View style={s.header}>
        ...
        <TouchableOpacity style={s.notifButton} onPress={() => navigation.navigate('Recent')}>
          <Ionicons name="time-outline" size={24} color={colors.onSurface} />
        </TouchableOpacity>
      </View>
      ...
      <View style={s.section}>
        ...
        <FlatList horizontal data={SUGGESTED_PLAYLISTS} keyExtractor={(i) => i.id} showsHorizontalScrollIndicator={false}
          ...
          renderItem={({ item }) => (
            <TouchableOpacity activeOpacity={0.8} onPress={() => handlePlaylistPress(item)} style={s.suggestedCard}>
              <LinearGradient colors={item.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.suggestedGradient}>
                <View style={s.decoCircle} /><View style={s.decoCircle2} />
                <View style={s.suggestedContent}>
                  <View style={s.suggestedIconWrap}><Ionicons name={item.icon} size={22} color="#fff" /></View>
                ...
                <View style={s.suggestedPlayBtn}>
                  {loadingPlaylist === item.id ? <ActivityIndicator size="small" color="#fff" /> : <Ionicons name="play" size={18} color="#fff" />}
                </View>
              </LinearGradient>
            </TouchableOpacity>
          )}
      ...
      {recentlyPlayed.length > 0 && (
        <View style={s.section}>
          <View style={s.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('Recent')}><Text style={s.seeAll}>See All</Text></TouchableOpacity>
          </View>
          <FlatList horizontal data={recentlyPlayed.slice(0, 10)} keyExtractor={(i) => i.id} showsHorizontalScrollIndicator={false}
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={s.recentItem} onPress={() => playSong(item, recentlyPlayed)}>
                ...
                <Text style={s.recentTitle} numberOfLines={1}>{item.name}</Text>
      ...
      {favorites.length > 0 && (
        <View style={s.section}>
          <View style={s.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('Favorites')}><Text style={s.seeAll}>See All</Text></TouchableOpacity>
          </View>
          <FlatList horizontal data={favorites.slice(0, 8)} keyExtractor={(i) => i.id} showsHorizontalScrollIndicator={false}
            contentContainerStyle={{ paddingHorizontal: Spacing.lg }}
            renderItem={({ item }) => (
              <TouchableOpacity style={s.favItem} onPress={() => playSong(item, favorites)}>
                <View>
                  <Image source={{ uri: item.image }} style={s.favImage} />
                  <View style={s.favHeart}><Ionicons name="heart" size={14} color={colors.primary} /></View>
                </View>
                <Text style={s.recentTitle} numberOfLines={1}>{item.name}</Text>
                <Text style={s.recentArtist} numberOfLines={1}>{item.artist}</Text>
              </TouchableOpacity>
...
const makeStyles = (c: ColorPalette) => StyleSheet.create({
  ...
  suggestedGradient: { width: '100%', height: CARD_WIDTH * 1.15, borderRadius: Radii.xl, padding: Spacing.lg, justifyContent: 'space-between', overflow: 'hidden' },
  decoCircle: { position: 'absolute', top: -20, right: -20, width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(255,255,255,0.12)' },
  decoCircle2: { position: 'absolute', bottom: -10, left: -15, width: 50, height: 50, borderRadius: 25, backgroundColor: 'rgba(255,255,255,0.08)' },
  suggestedContent: { flex: 1, justifyContent: 'flex-start' },
  suggestedIconWrap: { width: 40, height: 40, borderRadius: Radii.full, backgroundColor: 'rgba(255,255,255,0.22)', justifyContent: 'center', alignItems: 'center', marginBottom: Spacing.sm },
  ...
  suggestedPlayBtn: { alignSelf: 'flex-end', width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(0,0,0,0.25)', justifyContent: 'center', alignItems: 'center' },
  ...
  favHeart: { position: 'absolute', top: 8, right: 8, backgroundColor: 'rgba(0,0,0,0.6)', borderRadius: Radii.full, padding: 4 },
});
