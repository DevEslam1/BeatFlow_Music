...
export default function SearchScreen() {
  ...
  const handleSearch = useCallback(async (text: string) => {
    ...
    try { const songs = await searchSongs(text); setResults(songs); }
    catch { setResults([]); }
    setLoading(false);
  }, []);
  ...
  return (
    <View style={[s.container, { paddingTop: insets.top + Spacing.sm }]}>
      ...
      <View style={s.searchBar}>
        ...
        <TextInput style={s.searchInput} placeholder="Search songs, artists, albums..."
          placeholderTextColor={colors.onSurfaceVariant} value={query} onChangeText={handleSearch} returnKeyType="search" />
        {query.length > 0 && (
      ...
      <View style={s.filterContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}>
          {FILTERS.map((filter) => (
            <TouchableOpacity key={filter} style={[s.filterChip, activeFilter === filter && s.activeChip]} onPress={() => setActiveFilter(filter)}>
              <Text style={[s.filterText, activeFilter === filter && s.activeChipText]}>{filter}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
      ...
      {!hasSearched ? (
        <ScrollView contentContainerStyle={s.browseContent}>
          ...
          <View style={s.genreGrid}>
            {GENRES.map((genre) => (
              <TouchableOpacity key={genre.name}
                style={[s.genreCard, { backgroundColor: genre.color + '18', borderColor: genre.color + '40' }]}
                activeOpacity={0.7} onPress={() => handleSearch(genre.name)}>
                <View style={[s.genreIconWrap, { backgroundColor: genre.color + '28' }]}>
                  <Ionicons name={genre.icon} size={24} color={genre.color} />
                </View>
                <Text style={[s.genreName, { color: genre.color }]}>{genre.name}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </ScrollView>
...
const makeStyles = (c: ColorPalette) => StyleSheet.create({
  ...
  browseContent: { paddingHorizontal: GRID_PADDING, paddingTop: Spacing.sm, paddingBottom: 160 },
  browseTitle: { color: c.onSurface, fontSize: FontSizes.titleLg, fontWeight: '700', marginBottom: Spacing.lg },
  genreGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' },
  genreCard: { width: CARD_WIDTH, height: 110, borderRadius: Radii.xl, justifyContent: 'center', alignItems: 'center', borderWidth: 1, marginBottom: CARD_MARGIN * 2, overflow: 'hidden' },
  genreIconWrap: { width: 44, height: 44, borderRadius: Radii.full, justifyContent: 'center', alignItems: 'center', marginBottom: Spacing.sm },
  genreName: { fontSize: FontSizes.labelLg, fontWeight: '700', letterSpacing: 0.3 },
