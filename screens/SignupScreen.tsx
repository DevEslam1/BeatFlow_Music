...
export default function SignupScreen() {
  ...
  const handleSignup = async () => {
    ...
    try { const success = await signup(name, email, password); if (!success) setError('Signup failed.'); }
  ...
  return (
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={s.content}>
        ...
        <Text style={s.subtitle}>Join Melody and start listening</Text>

        <View style={s.form}>
          <View style={s.inputContainer}>
            <Ionicons name="person-outline" size={20} color={colors.onSurfaceVariant} />
            <TextInput style={s.input} placeholder="Full name" placeholderTextColor={colors.onSurfaceVariant} value={name} onChangeText={setName} />
          </View>
          ...
          <TouchableOpacity onPress={handleSignup} disabled={loading}>
            <LinearGradient colors={[colors.primary, colors.primaryContainer]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.signupButton}>
              {loading ? <ActivityIndicator color={colors.onPrimaryFixed} /> : <Text style={s.signupButtonText}>Create Account</Text>}
            </LinearGradient>
          </TouchableOpacity>
...
const makeStyles = (c: ColorPalette) => StyleSheet.create({
  ...
  title: { fontSize: FontSizes.headlineLg, fontWeight: '700', color: c.onSurface, marginBottom: Spacing.xs },
  subtitle: { fontSize: FontSizes.bodyLg, color: c.onSurfaceVariant, marginBottom: Spacing['4xl'] },
  form: { gap: Spacing.lg },
  ...
  signupButton: { height: 56, borderRadius: Radii.full, justifyContent: 'center', alignItems: 'center', marginTop: Spacing.sm },
  signupButtonText: { color: c.onPrimaryFixed, fontSize: FontSizes.titleMd, fontWeight: '700' },
